...
func TestOpenCodeAdapterProviderReadWrite(t *testing.T) {
	t.Parallel()
  ...
  "theme": "dark"
	...
	adapter := NewOpenCodeAdapter(configPath)
...
func TestOpenCodeAdapterProviderErrorsOnNonObjectProvider(t *testing.T) {
	t.Parallel()
...
func TestOpenCodeAdapterProviderErrorsOnMalformedScalarField(t *testing.T) {
	t.Parallel()
...
func TestOpenCodeAdapterMCPReadWrite(t *testing.T) {
	t.Parallel()
...
func TestOpenCodeAdapterMCPErrorsOnNonObjectTopLevel(t *testing.T) {
	t.Parallel()
...
func TestOpenCodeAdapterGetMCPServersErrorsOnMalformedFields(t *testing.T) {
	t.Parallel()
...
func TestOpenCodeAdapterMetadata(t *testing.T) {
	...
	if got := adapter.Tool(); got != "opencode" {
		t.Fatalf("Tool() = %q, want %q", got, "opencode")
	}
	...
	if files[0].Tool != "opencode" {
		t.Fatalf("ConfigFiles()[0].Tool = %q, want %q", files[0].Tool, "opencode")
	}
	if files[0].DocURL != "https://opencode.ai/docs/config" {
...
func readJSONFileMapForOpenCodeTest(t *testing.T, path string) map[string]any {
	t.Helper()
